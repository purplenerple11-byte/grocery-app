...
/* The lane past the last real list. A unique object, not a magic string:
   a string sentinel has to be un-typeable, and the obvious way to do that
   (a control-character prefix) puts a raw NUL in the source — which breaks
   grep and diffs, and which Postgres text columns reject outright, so it
   would fail the upsert the moment it leaked into an item's listStore.
   Object identity cannot collide and cannot be mis-encoded. Compared with
   === everywhere, and indexOf() in Store.nextList matches it by identity. */
const NEW_LIST = Object.freeze({ isNewListLane: true });

/* Navigation lanes = the real lists plus the create card on the end. Keeping
   the card in the lane array is what makes swiping BACK off it work for free,
   and gives it a page dot without a special case in the dots renderer. */
function listLanes() { return [...state.roster, NEW_LIST]; }

async function setCurrentList(name) {
  state.currentList = name;
  render();
  // Never persist the create card — it is a view, not a list. Restoring into
  // it on boot would open the app on a naming form.
  if (name === NEW_LIST) return;
...
function renderList() {
  ...
  const creating = state.currentList === NEW_LIST;
  const onList = creating ? [] : Store.itemsForList(state.items, state.currentList);
  const checkedCount = onList.filter((it) => it.checked).length;
  document.getElementById('list-store').textContent =
    creating ? '- new list' : (state.currentList ? `- ${state.currentList}` : '');
  document.getElementById('list-sub').textContent = creating
    ? 'Swipe back to cancel'
    : (onList.length ? `${onList.length} item${onList.length === 1 ? '' : 's'} · ${checkedCount} checked` : '');
  document.getElementById('complete-trip').hidden = creating || checkedCount === 0;
  // Nothing to add an item to until the list has a name.
  document.getElementById('add-form').hidden = creating;

  /* One dot says nothing — but the create card counts as a lane, so a single
     real list still gets dots the moment the card is reachable. */
  document.getElementById('list-dots').innerHTML = listLanes()
    .map((n) => `<i class="${n === NEW_LIST ? 'add' : ''}${n === state.currentList ? ' on' : ''}"></i>`)
    .join('');

  if (creating) {
    listEl.innerHTML = `
      <div class="new-list-card">
        <div class="new-list-plus" aria-hidden="true">＋</div>
        <h2>New list</h2>
        <p>Another store you shop at.</p>
        <input id="new-list-name" type="text" placeholder="e.g. BJ's Club"
               autocomplete="off" maxlength="60" aria-label="Name the new list">
        <div class="new-list-actions">
          <button type="button" id="new-list-cancel">Cancel</button>
          <button type="button" id="new-list-create" class="btn-clay">Create</button>
        </div>
      </div>`;
    return;
  }
...
(function () {
  ...
  /* One breath, not two animations. The halves are deliberately mirrored:
     same duration, same stagger, and easings that are each other's reflection
     — out accelerates away, in decelerates back, so the apex is the turn of
     a single motion rather than a seam between two.

     What made it chop: the halves used different durations (140/170) and
     different staggers (14/16), so the second half visibly re-started at a
     new tempo; and the DOM swap sat behind a setTimeout, which on a phone
     lands a frame or two late and drops a hole at the apex. The swap is now
     rAF-driven and the pause is gone.

     The overshoot survives — you chose it — but softened from 1.18 to 1.06.
     At 1.18 the list bobs, which reads as a bounce tacked onto the end; at
     1.06 it settles, which is what the end of a breath does. */
  const OUT_MS = 200, OUT_STAGGER = 16, OUT_EASE = 'cubic-bezier(.55, 0, .85, .25)';
  const IN_MS  = 200, IN_STAGGER  = 16, IN_EASE  = 'cubic-bezier(.15, .85, .4, 1.06)';
  ...
  /* .new-list-card and .empty are in here too: they are what the list area
     ...
     in with no motion at all while every other lane animated — the seam the
     animation exists to hide. */
  const parts = (el) => [...el.querySelectorAll('.row, .cat, .new-list-card, .empty')];
  const dockY = () => {
  ...
  window.switchList = function (dir) {
    if (animating) return;
    /* Swiping onto the create card is an ordinary lane change, so it gets the
       same inhale animation as any other — the card drops out of the block
       exactly the way a list does. Past the first list is still nothing. */
    const next = Store.nextList(listLanes(), state.currentList, dir);
    if (next === null) return;
    ...
    out.forEach((el, i) => {
      ...
      /* Opacity rides the back half of the move, mirrored by the in-phase
         below, so a row is solid while it travels and only thins as it
         reaches the block. Fading over the whole distance made rows vanish
         before they arrived, which is what read as a cut. */
      el.style.transition =
        `transform ${OUT_MS}ms ${OUT_EASE} ${d * outStep}ms, ` +
        `opacity ${Math.round(OUT_MS * 0.5)}ms linear ${d * outStep + OUT_MS * 0.5}ms`;
    ...
    setTimeout(() => {
      ...
      /* A forced reflow, deliberately, and NOT requestAnimationFrame.
         rAF is the textbook answer here, but it does not tick in a
         backgrounded or non-rendering tab — and every incoming row is
...
document.getElementById('list').addEventListener('click', async (e) => {
  if (e.target.closest('#new-list-cancel')) {
    setCurrentList(state.roster[state.roster.length - 1]);
    return;
  }
  if (e.target.closest('#new-list-create')) await createListFromCard();
});

document.getElementById('list').addEventListener('keydown', (e) => {
  if (e.key === 'Enter' && e.target.id === 'new-list-name') {
    e.preventDefault();
    createListFromCard();
  }
});

async function createListFromCard() {
  const input = document.getElementById('new-list-name');
  if (!input) return;
  const check = Store.canAddList(state.roster, input.value);
  if (!check.ok) { showBanner(check.reason); input.focus(); return; }
